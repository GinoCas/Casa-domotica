...
import { getDeviceById, getDeviceList, setBrightness, setDeviceState, setSpeed } from "@/lib/deviceController";
...
export const deviceService = {
  ...
  setDeviceState: async (deviceId: number, state: boolean): Promise<Result<any>> => {
    return await setDeviceState(deviceId, state);
  },
  
  setBrightness: async (deviceId: number, brightness: number): Promise<Result<any>> => {
