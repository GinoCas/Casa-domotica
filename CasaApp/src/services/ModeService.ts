...
import {
  ControlModeUseCase,
  GetModesUseCase,
} from "@/src/application/usecases/ModeUseCases";
...
export class ModeService {
  ...
  async controlMode(dto: ArduinoModeDto): Promise<Result<boolean>> {
    const useCase =
      this.container.getDependency<ControlModeUseCase>("controlMode");
    return await useCase.execute(dto);
  }

  async getModes(): Promise<Result<Mode[]>> {
    const useCase =
      this.container.getDependency<GetModesUseCase>("getAllModes");
